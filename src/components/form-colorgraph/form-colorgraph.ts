...
import { asOklch } from "../../functions/parser/asOklch.ts";
import { getStepColor } from "../../functions/shared/getStepColor.ts";
import { rafThrottle } from "../../functions/shared/rafThrottle.ts";
...
export default component<FormColorgraphProps>(
  ...
  (el, { all, first, useElement }) => {
    ...
    const moveThumb = rafThrottle((x: number, left: number, width: number) => {
      const color = {
        ...el.color,
    ...
    const effects = [
      ...
      () => {
        const setCanvasSize = rafThrottle((w: number) => {
          canvasSize.set(w);
        });
